...
## ⚙️ Config Improvements (`.env` + `moon.toml`)
...
- Warn on unrecognized keys (e.g., `MOON_DISTIL_PROVIDER` → "Did you mean `MOON_DISTILL_PROVIDER`?").
...
- Print the fully resolved config: defaults → `moon.toml` overrides → `.env` overrides.
- Shows exactly what moon is running with — invaluable for debugging.
- Secrets are masked in output.

### 4. Missing `.env` Warning
- On startup, if `.env` is not found, log: "`.env` not found — distill/embed features will be unavailable."
- Non-fatal — daemon still boots for basic operations.

---

## 🛡️ Safety Protocols: The Full Picture

### P0 — Must-Do (Prevents Real Failures)
...
- **Enforcement**: CLI verifies its Build UUID against the running daemon's. Mismatch → refuse "Reload" signals, prompt `moon restart --rebuild`.
...
- **The Protocol**: Liveness verification — lift the `embed.rs` lock pattern to the daemon level.
- **Enforcement**:
    - `.lock` file stores PID + start-timestamp + Build UUID.
    - `moon start` performs `kill(0, pid)` liveness check.
    - Stale lock (dead PID or mismatched Build UUID) → auto-remediate.

#### 3. Panic Guard in Daemon Loop
- **The Risk**: `run_once_with_options()` in `watcher.rs` is ~940 lines. Any panic inside it kills the daemon silently — no log, no alert, just gone.
- **The Protocol**: Wrap each watcher cycle in `std::panic::catch_unwind()`.
- **Enforcement**: On panic → log the backtrace, increment a panic counter in state, continue to next cycle. If 3 consecutive panics → halt and log `DAEMON_PANIC_HALT`.
...
- **Enforcement**: If JSON parse fails → backup corrupt file as `moon_state.json.corrupt.{timestamp}`, start with fresh defaults, log `WARN: state file corrupt, starting fresh`.

### P1 — Should-Do (Prevents Common Issues)
...
- **Enforcement**: Every command validates CWD. Wrong workspace → `ERR_OUT_OF_BOUNDS`.
...
- **Enforcement**: On signal → set flag, finish current cycle, flush state, exit cleanly.
...
#### 8b. Suppress No-Op Embed Logs ✅ (already fixed)
...
- **Location**: `watcher.rs` lines 1478–1498.
...
## ⚠️ Logic Issues to Resolve

### 1. `catch_unwind` vs `panic=abort`
- If `Cargo.toml` sets `panic = "abort"` in any profile, `std::panic::catch_unwind()` is a **no-op** — the process just dies.
...
- P3 says "3 consecutive panics → halt". But the counter reset condition is unspecified — does it reset after one good cycle? After N good cycles? Undefined behavior here could lead to either too-tolerant (never halts) or too-aggressive (halts after brief flap) behavior.
...
- P5 says "every command validates CWD". But diagnostic commands like `moon status` and `moon health` should work from *any* directory — users need them precisely when something is wrong.
...
### P0 (Immediate)
1. [ ] Add `build.rs` to generate compile-time Build UUID.
2. [ ] Extend daemon lockfile: PID + timestamp + Build UUID + liveness check.
3. [ ] Wrap `run_once_with_options()` in `catch_unwind()` with panic logging.
4. [ ] Add state file corruption recovery with auto-backup.

### P1 (Next)
5. [ ] Add CWD validation to all CLI commands.
6. [ ] Implement graceful shutdown with signal handling.
7. [ ] Audit all `Command::new()` sites for timeout coverage.
8. [ ] Add secret masking to all status/verify/log output.

### P2 (When Time Permits)
9. [ ] Add typo detection for `MOON_*` env vars.
10. [ ] Add `moon config --show` command.
11. [ ] Add missing `.env` startup warning.
12. [ ] Add log rotation.
13. [ ] Add `moon health` probe command.
14. [ ] Add `--dry-run` to watcher/distill/embed commands.
15. [ ] Introduce structured error codes.

## 📎 Related
- **LanceDB memory indexing**: Deferred to [mip-lanceDB.md](./mip-lanceDB.md).
- **Config unification (`.env` → `moon.toml`)**: Dropped — current split follows industry conventions.

---
*Created by Lilac Livint | Master's Maid*
*Revised 2026-02-28 — full safety picture + config improvements*
